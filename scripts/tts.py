...
REHABPANEL_INTL defaults to "1" (intentional — the whole project uses the
Singapore dashscope-intl endpoint, matching rehabpanel/qwen_client.py); set "0"
for the mainland endpoint. CosyVoice v2 streams over WebSocket, so we point both
the HTTP and the WebSocket base URLs at the intl region.
"""
import os
import sys
...
def main():
    ...
    try:
        if os.environ.get("REHABPANEL_INTL", "1") == "1":
            ...
            # CosyVoice v2 uses WebSocket — intl routing needs the ws endpoint too
